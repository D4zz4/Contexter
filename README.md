...
- Auf Android vorhandene YouTube-Untertitel direkt mit eingebettetem `yt-dlp` ohne API-Schlüssel abrufen; Deutsch oder Englisch wählen. Im Emulator wurde ein englisches Video-Transkript erfolgreich geladen. YouTube kann einzelne Abrufe begrenzen oder blockieren. Im Browser oder als Ausweichweg lassen sich `.vtt`- oder `.srt`-Dateien importieren.
