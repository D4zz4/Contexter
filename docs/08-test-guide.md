...
13. Ein Notebook mit mindestens zwei Quellen als „ZIP mit Einzeldateien“ exportieren und den Ordner `contexter` in Obsidian öffnen. In `sources/` sollten lesbare Dateinamen und in jeder geöffneten Notiz der Quellentitel als Überschrift erscheinen. `index.md` sollte zu den Quellen verlinken. `AGENTS.md` ist eine Anleitung zum Erstellen eines kuratierten OKF-Vaults; der Export erstellt noch keine solchen Konzepte. Dieses ZIP ist **nicht** die wiederherstellbare Bibliothekssicherung.
