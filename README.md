...
**Stand: 0.9.0-test.7.** Eine persönlich testbare Android-APK und eine Chromium-Erweiterung sind vorhanden. Die Android-App wurde auf einem echten Gerät mit YouTube-Untertiteln getestet. Dies ist noch keine öffentlich freigegebene 1.0: schwierige Referenzdateien und Live-Tests der optionalen Anbieter mit eigenen API-Schlüsseln stehen aus. Die ursprüngliche Planungsbaseline steht unter [docs/README.md](docs/README.md); [docs/07-implementation-status.md](docs/07-implementation-status.md) hält den aktuellen Stand und spätere Produktentscheidungen fest.

## Was die Testversion kann

Die Oberfläche lässt sich über das Zahnrad rechts oben zwischen Deutsch und Englisch umstellen. Der Papierkorb kann nach ausdrücklicher Bestätigung endgültig geleert werden. Notebook-ZIP-Exporte enthalten jetzt lesbare, titelbasierte Quelldateien für Obsidian und eine `AGENTS.md`-Anleitung zur kuratierten OKF-Weiterverarbeitung. Der Export erstellt selbst noch keine inhaltlich geprüften Konzepte; eine vollständige Contexter-Bibliothekssicherung ist ein anderes ZIP.
...
- Die gesamte lokale Bibliothek als Sicherungs-ZIP ausgeben und auf einem anderen Gerät **manuell** als neues Notebook importieren, zusammenführen oder ersetzen. Android bietet Contexter für ZIP-Dateien unter „Öffnen mit“ an. Abweichende Fassungen bleiben beim Zusammenführen als Konfliktkopien erhalten. Es gibt keinen automatischen Cloud-Abgleich.
